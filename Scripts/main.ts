        ...
        axios.get('/api/livesearch/all/' + ddr)
            .then(response => {
                ...
                for (var i = 0; i < response.data.length; i++) {
                    sb += `<div class="d-flex align-items-center border-bottom pb-4 mb-4 fade-in">
                     <a class="position-relative d-inline-block flex-shrink-0 bg-secondary rounded-1" href="${ response.data[i].url}">
                    <img src="/assets/images/search_thumbs/${response.data[i].image}.svg" width="70" alt="Product" /></a><div class="ps-3">
